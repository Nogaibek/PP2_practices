...
def save_session(player_id, score: int, level_reached: int,
                 username: str = "Player") -> bool:
    if not DB_AVAILABLE or player_id is None:
        _local_save(username, score, level_reached)
        return False
    try:
        with _cursor() as cur:
            cur.execute(
...
# ── локальный fallback (работает без Postgres) ──────────────────────────
import json as _json

_LOCAL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           "local_scores.json")

def _local_save(username: str, score: int, level: int):
    try:
        data = []
        ...
        data.append({
            ...
            "score":         score,
            "level_reached": level,
            "played_at":     str(datetime.datetime.now())[:10]
        })
        data.sort(key=lambda x: x["score"], reverse=True)
        with open(_LOCAL_PATH, "w") as f:
            _json.dump(data[:10], f, indent=2)
    except Exception as e:
        print(f"[db] local_save: {e}")

def _local_top10() -> list[dict]:
    try:
        if os.path.exists(_LOCAL_PATH):
            with open(_LOCAL_PATH) as f:
                return _json.load(f)
